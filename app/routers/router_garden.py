...
from app.schemas.schema_garden import GardenCreate, Garden as GardenPydantic
...
# 텃밭 업데이트 엔드포인트
@router.post("/api/garden", response_model=GardenPydantic)
def update_garden(garden_data: GardenCreate, db: Session = Depends(get_db)):

    db_garden = Garden(**garden_data.dict())
    db.add(db_garden)
    db.commit()
...
# 텃밭 정보 엔드포인트
@router.get("/api/mygarden", response_model=List[GardenPydantic])
